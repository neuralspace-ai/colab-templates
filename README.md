...
`https://<your-tunnel>.trycloudflare.com/api/v1`. If the field is blank, the
notebook falls back to the `PUBLIC_API_BASE_URL` environment variable. Quick
Cloudflare tunnel URLs can change after restart, so reopen or update the
notebook whenever the tunnel URL changes.
